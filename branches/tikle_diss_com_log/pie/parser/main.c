		...
		if (aff[i][0] == 0) {
			...
			if ((aff[i][1] == aff[foo][2]) && (aff[i][2] == aff[foo][1])) {
				...
				for (j=4;j<line_size;j+=4) {
					aff[foo][j-1] = 1;
					aff[i][j-1] = 1;
					printf("EVENTO %d: analisando linha %lu (in/out: %lu/%lu) com linha %d (in/out: %lu/%lu)\n", aff[i][j], foo, aff[foo][j+1], aff[foo][j+2], i, aff[i][j+1], aff[i][j+2]);
...
//	for (i=0;i<num_ips;i++) {
//		for (k=0;k<=line_size;k+=line_size) {
//			if (log_all[i][k] == 0) {
//				printf("verificando %lu e %lu com linha %d\n", log_all[foo][1], log_all[foo][2], i);	
//				if ((log_all[i][k+1] == log_all[foo][k+2]) && (log_all[i][k+2] == log_all[foo][k+1])) {
//					printf("verdadeiro\n");
//					log_all[i][k] = 1; 
//					for (j=4;j<4*num_events+3;j+=4) {
//						log_all[foo][j-1] = 1;
//						log_all[i][j-1] = 1;
//						printf("EVENTO %lu: analisando linha %d (in/out: %lu/%lu) com linha %d (in/out: %lu/%lu)\n", log_all[i][j], foo, log_all[foo][j+1], log_all[foo][j+2], i, log_all[i][j+1], log_all[i][j+2]);
//						if ((log_all[foo][j+1] != log_all[i][j+2]) || (log_all[foo][j+2] != log_all[i][j+1])) {
//							printf("Violaçao detectada no evento %lu\n", log_all[i][j]);
//							return 1;
//						}
//					}
//				}
...
//	printf("BIT | DADDR | SADDR\n");
//	for (i = 0; i < num_ips; i++) {
//		for (j=0;j<=line_size;j+=line_size) {
//			if (log_all[i][j] == 0) {
//				if ((log_all[i][j+1] != 0) && (log_all[i][j+2] != 0)) {
//					printf("enviando %lu e %lu\n", log_all[i][j+1], log_all[i][j+2]);
//					if (verifica(i, j, num_ips, num_events) == 1) exit(1);
//				}
//			}
//		}
//	}
//	printf("Experimento realizado com sucesso\n\n");
//	printf("\n\n");
//	printf("BIT | DADDR | SADDR\n");
//	for (i = 0; i < num_ips; i++) {
//		for (j=0;j<=line_size;j+=line_size) {
//			foo.s_addr = log_all[i][j+1];
//			bar.s_addr = log_all[i][j+2];
//			printf("%lu%s%s", log_all[i][j+0], inet_ntoa(foo), inet_ntoa(bar));
//			printf("%lu %lu %lu", log_all[i][j+0], log_all[i][j+1], log_all[i][j+2]);
//			for (k=3;k<line_size;k++) {
//				printf(" %lu ", log_all[i][j+k]);
//			}
