...
//		for (i = 0; log_counters[i] && i < log_size; i+=line_size)
//			if (log_counters[i] == ipip_hdr(sb)->saddr)
//				break;

//		log_counters[i+0] = ipip_hdr(sb)->saddr;
//		log_counters[i+3*trigger_flag+1] = trigger_flag;
//		log_counters[i+3*trigger_flag+2] += 1;
...
//		for (i = 0; log_counters[i] && i < log_size; i+=line_size)
//			if (log_counters[i] == iph->daddr)
//				break;

//		log_counters[i+0] = iph->daddr;
//		log_counters[i+3*trigger_flag+1] = trigger_flag;
//		log_counters[i+3*trigger_flag+3] += 1;
...
//	send_msg(sock_log, &addr_log, log_counters, sizeof(unsigned long) * log_size);
	send_msg(sock_log, &addr_log, log_counters, log_size);
...
//	size = send_msg(pie_sock->s_recv, &pie_sock->addr_recv, auth_msg, sizeof("pie:start"));
	...
	if (size < 0) {
		...
		if (num_ips) {
			line_size = 3 * (num_events) + 1;
			log_size = num_ips * line_size;
			log_counters = kcalloc(log_size, sizeof(long int), GFP_KERNEL | GFP_ATOMIC);
		}
		event_handler();
	} else { 
